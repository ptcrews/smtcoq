...
(* We're tagging points in proofs as potential goals for the SMT solver.
   Search for "(* Goal" to find these. We classify them as 
   T1: where simply encoding the lists as ADTs in the SMT solver 
       is expected to deliver results
   T2: where the solver should find the next lemma to be an abduct *)
Require Setoid.
...
(* Goal 5 T2 *)
(* Success!
Fail Timeout 10 (time abduce 1).
Tactic call ran for 1.521 secs (0.u,0.012s) (failure)
...
(Init.Nat.add ((length (A:=A)) l) 0) = ((length (A:=A)) l)
     assert (length l + 0 = length l). { now rewrite plus_n_O. }
     smt. *)
...
Section Elts.
  ...
  Proof.
    ...
    - (* Goal 6 T2 *) apply (proj1 (Nat.succ_le_mono _ _) (IHl x)).
  Qed.
...
Section ListOps.
  ...
  Proof.
    ...
    simpl.
    (* Goal 7 T2 *) rewrite app_nil_r; auto.

    intro y.
    simpl.
    rewrite (IHl y).
    (* Goal 8 T2 *) rewrite app_assoc; trivial.
  Qed.
  ...
  Proof.
    intro l; induction l as [| a l IHl].
    (* Goal 9 T2 *) simpl; auto.

    simpl. 
    (* Goal 10 T2 *) rewrite (rev_unit (rev l) a).
    rewrite IHl; auto.
  Qed.
  ...
  Proof.
    intros l l1 l2 Heq.
    rewrite <- (rev_involutive l). (* Goal 11 T2 *) rewrite Heq.
    apply rev_app_distr.
  Qed.
  ...
  Proof.
    ...
    (* Goal 13 T2 *)rewrite app_nil_r; trivial.
  Qed.
...
(*********************************************)
  ...
  - reflexivity.
  - rewrite IH. (* Goal 14 T2 *) apply app_assoc.
  Qed.
...
Section Map.
  ...
  Proof.
    ...
    (* Goal 16 T2 *)rewrite IHl1, app_assoc; reflexivity.
  Qed.
...
intros A B f l; induction l as [|x l IH]; simpl.
- reflexivity.
- (* Goal 17 T2 *) rewrite map_app, IH; reflexivity.
Qed.
...
End Fold_Right_Recursor.
  ...
  (** [filter] *)
    ...
    Proof.
      ...
      simpl. rewrite IHl. (* Goal 18 T2 *) rewrite filter_app. reflexivity.
    Qed.
...
Section length_order.
  ...
  Proof.
    unfold lel; intros.
    (* Goal 19 T2 *) now_show (length l <= length n).
    apply le_trans with (length m); auto with arith.
  Qed.

  Lemma lel_cons_cons : lel l m -> lel (a :: l) (b :: m).
  Proof.
    (* Goal 20 T1 *)unfold lel; simpl; auto with arith.
  Qed.

  Lemma lel_cons : lel l m -> lel l (b :: m).
  Proof.
    (* Goal 21 T1 *)unfold lel; simpl; auto with arith.
  Qed.

  Lemma lel_tail : lel (a :: l) (b :: m) -> lel l m.
  Proof.
    (* Goal 22 T1 *) unfold lel; simpl; auto with arith.
  Qed.
...
Section Cutting.
  ...
  Proof. induction n as [|k iHk]; intros l1 l2.
    - now simpl.
    - destruct l1 as [|x xs].
      * unfold firstn at 2, length. (* Goal 24 T3 *) rewrite 2!app_nil_l.
  ...
  Proof. induction n as [| k iHk];intros l1 l2.
    - unfold firstn at 2. rewrite <- plus_n_O, app_nil_r.
      ...
      unfold firstn at 2. (* Goal 25 T3 *) rewrite app_nil_r. apply firstn_all.
    - destruct l2 as [|x xs].
...
#[deprecated(since="8.12",note="Use skipn_all instead.")] Notation skipn_none := skipn_all.
  ...
  Proof.
    ...
    rewrite skipn_firstn_comm. now rewrite L.
  Qed.
  ...
  Proof.
    induction n.
    - intros l; simpl. (* Goal 26 T2 *) rewrite Nat.sub_0_r; reflexivity.
    - intro l; destruct l; simpl; auto.
  Qed.
  ...
  Proof.
    ...
    (* Goal 28 T3 *) rewrite rev_involutive. now rewrite rev_length.
  Qed.

  Lemma skipn_rev: forall x l,
      skipn x (rev l) = rev (firstn (length l - x) l).
  Proof.
    intros x l. rewrite firstn_skipn_rev. rewrite rev_involutive. 
    rewrite <-rev_length.
    destruct (Nat.le_ge_cases (length (rev l)) x) as [L | L].
    - rewrite skipn_all2; [apply Nat.sub_0_le in L | trivial].
      (* Goal 29 T3 *) rewrite L. rewrite Nat.sub_0_r. now rewrite skipn_all.
    - replace (length (rev l) - (length (rev l) - x))
         with (length (rev l) + x - length (rev l)).
      (* Goal 30 T2 *) rewrite minus_plus. reflexivity.
      (* Goal 31 T1 *) rewrite <- (Nat.sub_add _ _ L) at 2.
      rewrite <-!(Nat.add_comm x). now rewrite <-minus_plus_simpl_l_reverse.
  Qed.
...
Section Combining.
    ...
    Proof.
      ...
      (* Goal 32 T3 *) rewrite combine_length. simpl. rewrite Nat.min_0_r.
      reflexivity.
    Qed.
...
Section ReDun.
    ...
    - rewrite NoDup_cons_iff, Hrec, (count_occ_not_In decA). clear Hrec. split.
      ...
      + intro H; split.
        ...
        * intros x. specialize (H x). simpl in *. destruct (decA a x); auto.
          (* Goal 33 T1 *) now apply Nat.lt_le_incl.
  Qed.
  ...
  Proof.
    ...
    - (* Goal 34 T1 *) now apply Nat.le_antisymm.
    - destruct (Nat.le_gt_cases 1 n); trivial.
      + rewrite H; trivial.
      + (* Goal 35 T1 *) now apply Nat.lt_le_incl.
  Qed.
...
Section NatSeq.
  ...
  Proof.
    ...
    - (* Goal 36 T1 *) now rewrite Nat.add_0_r.
    - rewrite Nat.add_succ_r. now rewrite IHlen.
  Qed.
  ...
  Proof.
   ...
   rewrite <- seq_app.
   (* Goal 37 T3 *) rewrite <- plus_n_Sm, <- plus_n_O; reflexivity.
  Qed.
...
Section Repeat.
  ...
  Proof.
    ...
    - repeat split. (* Goal 38 T2 *) now rewrite repeat_length.
...
simpl; rewrite IHl1.
(* Goal 39 T1 *) apply Nat.add_assoc.
Qed.
...
Proof.
intro l1; induction l1 as [|a l1 IHl1]; intros l2; [ reflexivity | ].
simpl; rewrite IHl1. (* Goal 40 T2 *) now rewrite Nat.max_assoc.
Qed.
...
- destruct l.
  + repeat constructor.
    simpl in H. (* Goal 41 T2 *) now rewrite Nat.max_0_r in H.
  + apply Nat.max_lub_lt_iff in H.
    now constructor; [ | apply IHl ].
- destruct l; inversion_clear H as [ | ? ? Hlt HF ].
  + simpl. (* Goal 42 T2 *) now rewrite Nat.max_0_r.
  + apply IHl in HF.
