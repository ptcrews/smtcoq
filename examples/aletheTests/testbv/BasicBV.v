...
Require Import SMTCoq.bva.BVList Int63 ZArith.
...
Section BVTest.
  ...
  Goal forall (a: bitvector 64), bv_eq a a.
  Proof. smt. Qed.

  (* This proof is broken; not sure why *)
  Goal forall (a1 a2: bitvector 32), bv_eq (bv_add a1 a2) (bv_add a2 a1).
  Proof. intros. cvc5. Admitted.

  (* TODO: Try bv transitivity*)
  (* TODO: Work on distinct elim, then bit vectors *)
  ...
  Goal forall (bv1 bv2 bv3: bitvector 64),
    bv_ult bv1 bv2 -> bv_ult bv2 bv3 -> bv_ult bv1 bv3.
  Proof. cvc5. smt. Qed.

End BVTest.

Section BVTest_Zbits.
  Import BVList.BITVECTOR_LIST.
  Local Open Scope bv_scope.

  Lemma eqmod_refl: forall (x: bitvector 64), bv_eq x x.
  Proof. smt. Qed.

  (* TODO: Is this needed? *)
  Lemma eqmod_refl2: forall (x y: bitvector 64), x = y -> bv_eq x y.
  Proof. smt. Qed.

  Lemma eqmod_sym: forall (x y: bitvector 64), bv_eq x y -> bv_eq y x.
  Proof. smt. Qed.

  Lemma eqmod_trans: forall (x y z: bitvector 64),
    bv_eq x y -> bv_eq y z -> bv_eq x z.
  Proof. smt. Qed.

  Lemma eqmod_small_eq:
    forall (x y: bitvector 64), bv_eq x y -> 0 <= x < pow2 64 -> 0 <= y < pow2 64 -> x = y.
  Proof.
