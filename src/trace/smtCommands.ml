...
let make_proof call_solver i j env rt ro ra_quant rf_quant l ls_smtc =
  let root = SmtTrace.mkRootV [l] in
  call_solver i j env rt ro ra_quant rf_quant (root,l) ls_smtc
...
let core_tactic call_solver i j solver_logic rt ro ra rf ra_quant rf_quant vm_cast lcpl lcepl env sigma concl =
  ...
  let (body_cast, body_nocast, cuts) =
    if ((CoqInterface.eq_constr b (Lazy.force ctrue)) ||
      ...
      let max_id_confl = make_proof call_solver i j env rt ro ra_quant rf_quant nl lsmt in
...
let tactic call_solver i j solver_logic rt ro ra rf ra_quant rf_quant vm_cast lcpl lcepl =
  CoqInterface.tclTHEN
    Tactics.intros
    (CoqInterface.mk_tactic (core_tactic call_solver i j solver_logic rt ro ra rf ra_quant rf_quant vm_cast lcpl lcepl))
