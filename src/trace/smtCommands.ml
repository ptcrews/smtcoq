...
let make_proof i j k call_solver env rt ro ra_quant rf_quant l ls_smtc =
  let root = SmtTrace.mkRootV [l] in
  call_solver i j k env rt ro ra_quant rf_quant (root,l) ls_smtc
...
let core_tactic call_solver i j k solver_logic rt ro ra rf ra_quant rf_quant vm_cast lcpl lcepl env sigma concl =
  ...
  let (body_cast, body_nocast, cuts) =
    if ((CoqInterface.eq_constr b (Lazy.force ctrue)) ||
      ...
      let max_id_confl = make_proof call_solver i j k env rt ro ra_quant rf_quant nl lsmt in
...
let tactic call_solver i j k solver_logic rt ro ra rf ra_quant rf_quant vm_cast lcpl lcepl =
  CoqInterface.tclTHEN
    Tactics.intros
    (CoqInterface.mk_tactic (core_tactic call_solver i j k solver_logic rt ro ra rf ra_quant rf_quant vm_cast lcpl lcepl))
...
let abduct_string env rt ro ra rf =
  let t_i = make_t_i rt in
  function
  | List [Atom "define-fun"; Atom _; List []; _; expr] ->
      smt2_sexpr_to_coq_string env t_i ra rf expr
  | _ -> CoqInterface.error ("Could not reconstruct abduct")

(* Print result of a sygus-stream abduct query represented as a SExpr.t list *)
let abduct_strings env rt ro ra rf l =
  List.map (abduct_string env rt ro ra rf) l